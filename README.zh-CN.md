...
> 浏览器里的多机、多会话共享终端。选「机器 → 会话」开终端,任意设备接入,所有客户端**字节级同步**。

[English](README.md) · **中文**

一个轻量的自托管终端中枢。**Hub** 跑在你的服务器 / NAS 上 —— 负责网页、登录、字节中继,自己从不打开 shell;每台电脑跑一个 **Agent**,主动向 Hub *拨号出站*、注册自己,并按需在本机开 zsh。手机、平板、另一台电脑都能接入同一个会话,看到完全一致的画面。

## 特性

- 🖥️ **多机多会话** —— 一个网页管理所有机器上的所有 zsh 会话。
- 🔄 **字节级同步** —— 多端连同一会话,输入输出实时一致;新接入者立即看到当前屏(滚屏缓存)。
- 🔌 **NAT 友好** —— Agent 主动出站拨号,只需 Hub 可达,无需给每台机器开入站端口。
- 💾 **会话持久化** —— Agent 重启自动恢复会话列表与历史(shell 本身是新进程)。
- 🔐 **两层鉴权** —— 浏览器走 OIDC 单点登录(如 Synology),机器走一次性配对 token。
...
```
   浏览器(任意设备)──WSS + OIDC 登录──▶  Hub(服务器 / NAS, Docker)
                                            │  网页 + 鉴权 + 机器注册表 + 字节中继
                       Agent 主动拨号 ▲      │  (自己从不开 shell)
           ┌───────────────┬──────────────┘
           ▼               ▼
       Agent(Mac)     Agent(台式机) …    各自开本机 zsh PTY,桥接给 Hub
```

- **Hub**(`hub.js`):服务网页、OIDC 登录、机器注册 / 配对,并在浏览器与目标机器之间来回中继字节。为每个会话缓存滚屏(`SCROLLBACK` 字节),新浏览器接入即见当前屏。不依赖 node-pty(不开 PTY),所以容器镜像是纯 JS、无原生编译。
- **Agent**(`agent.js`):每台电脑一个常驻进程,拨号 `wss://<hub>/agent`(NAT 友好,只需 Hub 可达),持有本机会话,经一条 WebSocket 多路复用全部会话。断线指数退避重连,收到 SIGTERM 优雅刷盘。
...
```bash
cp .env.example .env      # 填入 AUTH_SECRET / OAUTH2_* 等(见下方表格)
docker compose up -d --build
```

`docker-compose.yml` 起两个容器(同一镜像):

- **hub** → 发布 `7654`(把你的反代指到这里),挂载 `./data` 存机器注册表。
- **usage** → 内部 `7655`,Claude 用量监控服务,仅 hub 经 compose 网络访问(不对外发布端口)。可选。

**反向代理**:把一个子域名整根转发到 Hub 的 `7654`,**务必开启 WebSocket 透传**并签 TLS。前端用绝对路径(`/ws`、`/api`),所以**不能放在子路径下**,必须独立子域名整根转发。
...
- 注册表 `data/machines.json` 在挂载卷上,容器重建不丢。
